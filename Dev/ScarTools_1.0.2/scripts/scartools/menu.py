...
def _run_topology_qa(*_):
    try:
        ...
        for node, data in results.items():
            short = node.split("|")[-1]
            if data["clean"]:
                emit_log("{}: Clean (No topology issues)".format(short), level="success", source="TopologyQA")
            else:
                ...
                emit_log("{}: ISSUES -> {} non-manifold verts, {} non-manifold edges, {} lamina faces, {} intermediate shapes".format(short, nm_v, nm_e, lam, inter), level="warning", source="TopologyQA")
